...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call Protcoin with fAggressive=true to
    // get as much data as possible.
    // Rewrite protcoind data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> protcoindData;
    bool allOK = dbenv.Protcoin(newFilename, true, protcoindData);
    if (protcoindData.empty()) {
        LogPrintf("Protcoin(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("Protcoin(aggressive) found %u records\n", protcoindData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, protcoindData) {
        if (fOnlyKeys) {
