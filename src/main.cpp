...
#if defined(NDEBUG)
#error "Prot cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uProt) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight >=       2 && nHeight <=    80) {nSubsidy =       0.00 * COIN;}
    else if (nHeight >=    81 && nHeight <=  43299) {nSubsidy =        5.00 * COIN;}
    ...
    else if (nHeight >= 172900 && nHeight <= 216099) {nSubsidy =        25.00 * COIN;}
    else if (nHeight >= 216100 && nHeight <= 259299) {nSubsidy =        30.00 * COIN;}
    else if (nHeight >= 259300 && nHeight <= 345698) {nSubsidy =        20.00 * COIN;}
    else if (nHeight >= 345699 && nHeight <= 432098) {nSubsidy =        15.00 * COIN;}
    else if (nHeight >= 532099 && nHeight <= 518498) {nSubsidy =        10.00 * COIN;}
    else if (nHeight >= 518499 && nHeight <= 1036887) {nSubsidy =        8.00 * COIN;}
    else if (nHeight >= 1036888 && nHeight <= 16804888) {nSubsidy =       6.00 * COIN;}
    else if (nHeight >= 16804889                     ) {nSubsidy =        1.00 * COIN;}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        ...
        return nSubsidy;
         ...
         if (nHeight >=       1 && nHeight <=    80)  {nSubsidy = 0;}
    else if (nHeight >=     101 && nHeight <=  16804888)  {nSubsidy = blockValue * 0.80;}
...
{
    RenameThread("protcoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // PROTCOIN: We use certain sporks during IBD, so check to see if they are
        ...
        vector<CInv> vInvWait;
        {
            ...
            BOOST_FOREACH (const CInv& inv, pto->vInventoryToSend) {
                ...
                if (inv.type == MSG_TX && !fSendTrickle) {
                    // 1/4 of tx invs prot to all immediately
                    static uint256 hashSalt;
                    if (hashSalt == 0)
                        hashSalt = GetRandHash();
