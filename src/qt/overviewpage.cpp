...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::PROT)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount protAvailableBalance = balance - immatureBalance - nLockedBalance;

    // PROT Watch-Only Balance
    CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance;
    CAmount nAvailableWatchBalance = watchOnlyBalance - watchImmatureBalance - nWatchOnlyLockedBalance;

    // PROT labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, protAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // PROT Available
    bool showPROTAvailable = settingShowAllBalances || protAvailableBalance != nTotalBalance;
    bool showWatchOnlyPROTAvailable = showPROTAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showPROTAvailable || showWatchOnlyPROTAvailable);
    ui->labelBalance->setVisible(showPROTAvailable || showWatchOnlyPROTAvailable);
    ui->labelWatchAvailable->setVisible(showPROTAvailable && showWatchOnly);

    // PROT Pending
    bool showPROTPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyPROTPending = showPROTPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showPROTPending || showWatchOnlyPROTPending);
    ui->labelUnconfirmed->setVisible(showPROTPending || showWatchOnlyPROTPending);
    ui->labelWatchPending->setVisible(showPROTPending && showWatchOnly);
    ...
    // PROT Locked
    bool showPROTLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyPROTLocked = showPROTLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showPROTLocked || showWatchOnlyPROTLocked);
    ui->labelLockedBalance->setVisible(showPROTLocked || showWatchOnlyPROTLocked);
    ui->labelWatchLocked->setVisible(showPROTLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("PROT")
    updateDisplayUnit();
}
